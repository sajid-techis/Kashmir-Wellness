...
import { getLabsThunk } from "../../features/labs/labSlice"; // Import the thunk for fetching labs
import { Link } from "react-router-dom";
import { AiOutlineCalendar, AiOutlineClockCircle, AiOutlineCheckCircle } from "react-icons/ai";

const UserLabAppointments = ({ patientId }) => {
  ...
  useEffect(() => {
    if (patientId) {
      dispatch(getLabAppointmentsThunk(patientId)); 
    }
  }, [dispatch, patientId]);

  // Fetch lab details based on labId for each appointment
  useEffect(() => {
    if (appointments.length > 0) {
      const labIds = [...new Set(appointments.map(appointment => appointment.labId))]; 
      labIds.forEach(labId => {
        dispatch(getLabsThunk({ id: labId })); 
      });
    }
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 bg-gray-50 rounded-2xl shadow-lg">
      <h2 className="text-3xl sm:text-4xl font-bold mb-6 text-center text-gray-800">Lab Appointments</h2>
      {appointments.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {appointments.map((appointment) => {
            const labDetail = labs.find(lab => lab._id === appointment.labId); 
            return (
              <div
                key={appointment._id}
              ...
              >
                <h3 className="text-xl sm:text-2xl font-semibold mb-2 text-indigo-600">
                  Appointment with {labDetail ? labDetail.name : "Loading..."} 
                </h3>
                <p className="text-gray-500 flex items-center gap-1 sm:gap-2 mb-1 sm:mb-2">
                  <AiOutlineCalendar /> <strong>Date:</strong> {new Date(appointment.date).toLocaleDateString()}
                </p>
                <p className="text-gray-500 flex items-center gap-1 sm:gap-2 mb-1 sm:mb-2">
                ...
                <Link
                  to={`/labAppointment/${appointment._id}`}
...
export default UserLabAppointments;
